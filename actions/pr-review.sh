...
if [[ $# -lt 2 ]]; then
  echo "Usage: $0 <repo> <branch> [base-branch]" >&2
  echo "  repo: repo name or path (names resolve to ~/GitHub/Code/<repo>)" >&2
  echo "  branch: branch to review" >&2
  echo "  base-branch: optional, default main" >&2
  exit 1
fi
...
if [[ -d "$REPO_ARG" ]]; then
  ...
  if [[ ! -d "$REPO_PATH" ]]; then
    echo "Error: repo not found: $REPO_ARG (tried $REPO_PATH)" >&2
    exit 1
  fi
...
if [[ -n "$(git -C "$REPO_PATH" status --porcelain)" ]]; then
  echo "Error: working tree is dirty in $REPO_PATH. Stash or commit first." >&2
  exit 1
fi
...
git -C "$REPO_PATH" checkout "$BRANCH" 2>/dev/null || { echo "Error: could not checkout branch: $BRANCH" >&2; exit 1; }
...
echo "FULL_CHANGES=${REVIEW_DIR}/full-changes.md"
